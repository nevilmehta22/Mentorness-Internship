...
CREATE VIEW unit_price_summ AS
SELECT 
...
CREATE VIEW purchase_total_summ AS
SELECT 
    MIN(purchase_total) AS min_purchase,
    MAX(purchase_total) AS max_purchase,
	round(AVG(purchase_total::numeric),2) AS avg_purchase_total
FROM walmart;

SELECT * FROM purchase_total_summ

-- Exploring cogs
CREATE VIEW cogs_summ AS
SELECT 
    MIN(cogs) AS min_cogs,
    MAX(cogs) AS max_cogs,
	round(AVG(cogs::numeric),2) AS avg_cogs
FROM walmart;

SELECT * FROM cogs_summ

-- Exploring gross_income
CREATE VIEW income_summ AS
SELECT 
    MIN(gross_income) AS min_income,
    MAX(gross_income) AS max_income,
	round(AVG(gross_income::numeric),2) AS avg_income
FROM walmart;

SELECT * FROM income_summ

-- Exploring rating_summary
CREATE VIEW rating_summ AS
SELECT 
